    ...
    if request.POST:
        ...
        if not commute_form.is_valid():
            ...
            if leg_formset_WRTW.is_valid() and leg_formset_WRFW.is_valid() and leg_formset_NormalTW.is_valid() and leg_formset_NormalFW.is_valid():
                ...
                request.session['strava_bike_total'] = str(bikeTotal)
...
def stravaupload(request):

    bikeTotal = int(request.session['strava_bike_total'])

    params = {'access_token': request.session['strava_access_token'], 'name':'GSI Walk/Ride Day Commute', 'type':'Ride', 'start_date_local': datetime.datetime.now(), 'elapsed_time': bikeTotal * 60 }

    r = requests.post("https://www.strava.com/api/v3/activities", data=params)
    ...
    return HttpResponseNotFound('<h1>Page not found ' + str(r.status_code) + '</h1>')
